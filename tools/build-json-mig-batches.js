...
function writeSpeciesChunks(size = 250) {
  fs.mkdirSync("tmp-mig/species-json", { recursive: true });
  let n = 0;
  ...
  select * from jsonb_to_recordset('${json.replace(/'/g, "''")}'::jsonb)
...
on conflict (dex) do update set
  ...
  spawn_weight = s.spawn_weight;`;
    ...
    fs.writeFileSync(`tmp-mig/species-json/${name}.sql`, sql);
    console.log("species-json", name, chunk.length, Buffer.byteLength(sql));
  }
}

function writeFormsChunks(size = 300) {
  fs.mkdirSync("tmp-mig/forms-json", { recursive: true });
  fs.writeFileSync(
    "tmp-mig/forms-json/00-delete.sql",
...
on conflict (dex, form_key, gender, shiny) do update set
  ...
  enabled_in_play = excluded.enabled_in_play;`;
    const name = String(++n).padStart(2, "0");
    fs.writeFileSync(`tmp-mig/forms-json/${name}.sql`, sql);
    console.log("forms-json", name, chunk.length, Buffer.byteLength(sql));
  }
}
