-- Phase 10 forward reconciliation — authoritative final state.
--
-- Purpose: bring ANY database that already has the original Phase 10 objects
-- (tables/RPCs from 20260917070000 / 20260917071000, whether or not those
-- versions were recorded in schema_migrations) to the live Phase 10 final
-- schema/function/security state.
--
-- Non-destructive: CREATE TABLE IF NOT EXISTS, CREATE OR REPLACE, GRANT/REVOKE.
-- Does not drop tables, rewrite schema_migrations, or mutate trainer data.
-- Safe to apply on production that already matches this state (no-op replace).
--
-- DO NOT re-run 20260917070000 or 20260917071000 against production. Those
-- files contain the originally manually-applied bodies. 70000's
-- launch_community_round and spawn-adjacent helpers predate the species
-- spawn guard. See PHASE10_MIGRATION_NOTE.md.
--
