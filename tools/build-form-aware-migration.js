/**
 * Build supabase migration SQL for form-aware Admin / Special Event identity.
 * Reads data/pokemon-forms-seed.json produced by import-organized-form-sprites.js.
 *
 * Usage: node tools/build-form-aware-migration.js
 */
const fs = require("fs");
const path = require("path");

const ROOT = path.join(__dirname, "..");
const seed = JSON.parse(fs.readFileSync(path.join(ROOT, "data", "pokemon-forms-seed.json"), "utf8"));
...
const values = seed
  .map((r) => {
    return `(${r.pokemon_form_id}, ${r.dex}, ${sqlStr(r.form_key)}, ${sqlStr(r.form_label)}, ${sqlStr(r.kind)}, ${r.is_base}, ${!!r.has_front}, ${!!r.has_shiny_front}, ${!!r.has_female_front}, ${!!r.has_back}, ${!!r.admin_targetable}, ${!!r.event_targetable}, ${!!r.normal_encounter_enabled}, ${sqlStr(r.asset_status)}, ${!!r.origin_gen1})`;
  })
  .join(",\n");

const out = `-- Form-aware Pokémon identity for Admin / Special Event encounters (Kanto v1.0).
...
const dest = path.join(ROOT, "supabase", "migrations", "20260918050000_form_aware_identity.sql");
fs.writeFileSync(dest, out);
console.log(`Wrote ${dest} (${seed.length} forms, ${Buffer.byteLength(out)} bytes)`);
