...
Deno.serve(async (req) => {
  ...
  if (staffError || !catalog) {
    return json({ ok: false, message: "Only staff can upload store sprites." }, 403);
  }
  ...
  if (!contentBase64) {
    return json({ ok: false, message: "Choose an image to upload." }, 400);
  }
  if (contentBase64.length > 1_200_000) {
    return json({ ok: false, message: "Keep sprites under 900 KB." }, 400);
  }

  const admin = createClient(supabaseUrl, service);
  const { data: token, error: tokenError } = await admin.rpc("store_github_token");
  if (tokenError || !String(token || "").trim()) {
    return json({ ok: false, message: "Owner needs to save a GitHub token on Staff tools first." }, 400);
  }
  ...
  if (!put.ok) {
    ...
    return json({ ok: false, message: detail }, 400);
  }
