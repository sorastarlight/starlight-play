...
test("Dialogue pages then in-place fade to result card", () => {
  assert(js.includes("evo-dialogue-gba"), "gba dialogue class missing");
  assert(js.includes("beginDialogueFlow"), "dialogue result flow missing");
  assert(js.includes("typeIntro"), "intro typewriter missing");
  assert(js.includes("typePage"), "page typewriter missing");
  assert(js.includes("full.slice(0, i + 1)"), "letter-by-letter typing missing");
  assert(js.includes("showResultCard"), "result card reveal missing");
  assert(js.includes("data-evo-result"), "result card markup missing");
  assert(js.includes("evo-stage-shell"), "shared shell missing");
  ...
  assert(css.includes(".evo-stage-shell"), "shared shell CSS missing");
  assert(css.includes(".evo-fanfare.is-result"), "result fade CSS missing");
  ...
  assert(view.includes("New Pokédex entry!"), "dex dialogue page missing");
  assert(view.includes("Trainer XP!"), "xp dialogue page missing");
});
...
test("Reduced-motion stays non-mutating", () => {
  ...
  assert(js.includes("beginDialogueFlow"), "skip/end still presentation-only");
});
