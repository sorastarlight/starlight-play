-- Remove the Pokémon Genderbending Sprite Pack.
...
as $$
  select coalesce(p_id, '') in (
    ...
    'ash-sinnoh', 'ash-unova', 'ash-kalos', 'ash-alola',
...
delete from private.store_items
where sku = 'avatar-genderbend' or extra->>'pack' = 'genderbend';

delete from private.store_assets
where filename in (
  'thumbnail_pokemongenderbending.png',
  'serenadress01.png',
  'hikaridress01.png'
);

update public.profiles
set owned_avatar_packs = array_remove(coalesce(owned_avatar_packs, '{}'::text[]), 'genderbend')
where 'genderbend' = any (coalesce(owned_avatar_packs, '{}'::text[]));

update public.profiles
set trainer_sprite = 'ash'
where trainer_sprite in (
  'ashley',
  'ash-ashley',
  'ashley-crossdress-alola',
  'ashley-crossdress-unova',
  'ashley-crossdress-sinnoh',
  'serena-crossdress'
);
