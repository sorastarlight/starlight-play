-- Anime Genderbending Pack: Ashley looks plus Serena Crossdress.
...
as $$
  select coalesce(p_id, '') in (
    ...
    'ashley-crossdress-alola', 'ashley-crossdress-unova', 'serena-crossdress',
...
set owned_avatar_packs = array_append(coalesce(owned_avatar_packs, '{}'::text[]), 'genderbend')
where trainer_sprite in (
  'ashley', 'ash-ashley',
  'ashley-crossdress-alola', 'ashley-crossdress-unova',
  'serena-crossdress'
)
  and not ('genderbend' = any (coalesce(owned_avatar_packs, '{}'::text[])));
